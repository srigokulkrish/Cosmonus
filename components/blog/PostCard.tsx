...
export function PostCard({ post }: { post: Post }) {
  return (
    <Link href={`/company/blog/${post.slug}`} className="row flex flex-col gap-3">
      <MediaPanel tone={post.cover.tone} label={post.cover.label} src={post.cover.image} decorative sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="mb-2 h-[200px] rounded-media" />
      <PostMeta post={post} />
...
export function FeaturedPost({ post, label }: { post: Post; label: string }) {
  return (
    <Link href={`/company/blog/${post.slug}`} className="row grid grid-cols-1 items-center gap-x-10 gap-y-6 lg:grid-cols-2">
      <MediaPanel tone={post.cover.tone} label={post.cover.label} src={post.cover.image} decorative className="h-[260px] rounded-card sm:h-[340px] lg:h-[420px]" />
