 ...
 * reduced motion) and shift a little further when a parent `group` is hovered. Fills its parent, which needs `relative isolate
 * overflow-hidden`. `dim` adds an even shade for large areas of white text (keeps it above 4.5:1); `bloom={false}`
...
export function MeshBackdrop({ palette, dim = false, bloom = true }: { palette: MeshPalette; dim?: boolean; bloom?: boolean }) {
  ...
  const field = "mesh-field absolute rounded-full blur-[64px]";
  return (
    <span aria-hidden="true" className="absolute inset-0 -z-10 overflow-hidden" style={{ backgroundColor: deep }}>
      <span className={`${field} mesh-a ${drift} -top-1/3 -right-1/4 h-[110%] w-[80%] group-hover:-translate-x-8 group-hover:translate-y-6`} style={{ background: mid }} />
      <span className={`${field} mesh-b ${drift} top-[5%] right-[5%] h-[70%] w-[55%] opacity-75 group-hover:-translate-x-10`} style={{ background: light }} />
      <span className={`${field} mesh-c ${drift} top-1/4 -left-1/3 h-[90%] w-[75%] opacity-90 group-hover:translate-x-8`} style={{ background: deep }} />
      <span className={`${field} mesh-d ${drift} -bottom-1/2 left-0 h-[90%] w-[110%] group-hover:-translate-y-6`} style={{ background: shadow }} />
